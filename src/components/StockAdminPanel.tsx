...
import { brandInfo, combos, products } from "@/data/site";
...
export function StockAdminPanel() {
  ...
  const stockById = useMemo(() => stockRecordsToMap(stockRecords), [stockRecords]);
  ...
  const loadStocks = useCallback(async () => {
    ...
    try {
      const { data, error } = await fetchStockRecords();
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await saveStockRecords(stockRecords);
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mt-5 grid gap-3">
        {editableItems.map((item) => {
          ...
          const stockLabel = getStockLabel(stockValue);
          const unavailable = isOutOfStock(stockValue);
