...
import type { Combo } from "@/dados/conteudo-site";
...
type PropriedadesCartaoCombo = {
  combo: Combo;
};

export function CartaoCombo({ combo }: PropriedadesCartaoCombo) {
  const { addItem } = useCarrinho();
  const effectiveStock = useValorEstoque(combo.id, combo.stock);
  const stockLabel = obterRotuloEstoque(effectiveStock);
  const comboIsOutOfStock = estaSemEstoque(effectiveStock);
    ...
    <article className="sweet-card tap-soft overflow-hidden rounded-[1.7rem] bg-[var(--color-surface)] shadow-sm transition hover:-translate-y-1.5 hover:shadow-brand sm:rounded-3xl">
      {combo.image ? (
        <div className="relative overflow-hidden rounded-t-[1.65rem] sm:rounded-t-[1.95rem]">
          <Image
            src={caminhoAsset(combo.image.src)}
            alt={combo.image.alt}
            width={720}
            height={520}
