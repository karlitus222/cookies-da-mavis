...
import { resolverValorEstoque, useMapaEstoque } from "./ProvedorEstoque";

type PropriedadesDestaqueSabores = {
  products: Product[];
};

export function DestaqueSabores({ products }: PropriedadesDestaqueSabores) {
  ...
  const stockById = useMapaEstoque();
  if (!initialProduct) {
    return null;
  }
  const selectedProduct =
    products.find((product) => product.id === selectedId) ?? initialProduct;
  const selectedStock = resolverValorEstoque(
    stockById,
    selectedProduct.id,
    selectedProduct.stock
  );
  const selectedStockLabel = obterRotuloEstoque(selectedStock);
  const selectedIsOutOfStock = estaSemEstoque(selectedStock);
    ...
    <div className="sweet-panel scroll-reveal mt-7 overflow-hidden rounded-[1.75rem] border border-[var(--color-primary)]/10 bg-[var(--color-background)]/94 p-3 shadow-sm sm:mt-10 sm:rounded-[2.5rem] sm:p-5 lg:grid lg:grid-cols-[0.9fr_1fr] lg:gap-6 lg:p-6">
      ...
      >
        <Image
          ...
          height={760}
          src={caminhoAsset(selectedProduct.image.src)}
          width={900}
        />
      ...
      <div className="mt-4 flex flex-col justify-center sm:mt-5 lg:mt-0">
        ...
        <div className="mobile-pill-scroll mt-5 flex snap-x gap-2 overflow-x-auto pb-1 sm:grid sm:grid-cols-6 sm:gap-3 sm:overflow-visible sm:pb-0">
          {products.map((product) => {
            ...
            const productStock = resolverValorEstoque(
              stockById,
              product.id,
              product.stock
            );
            const productIsOutOfStock = estaSemEstoque(productStock);
